...
const ENSAYOS = [
  ...
  ['2026-07-15-grupal-nico', 'Miércoles 15 de julio — Ensayo Nico + Jefe + Compañero de trabajo + Amigo de Jona + Negro'],
...
function doPost(e) {
  ...
  const headers = ['Timestamp', 'Personaje', 'Actor/a', 'Contacto', 'Comentarios'].concat(ENSAYOS.map(item => item[1]));
  ...
  const row = [new Date(), data.personaje || '', data.actor || '', data.contacto || '', data.comentarios || ''];
