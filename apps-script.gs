...
const ENSAYOS = [
  ...
  ['2026-07-15-grupal-nico', 'Miércoles 15 de julio — Ensayo Nico + Alexis + Compañero de taller de Nico + Amigo de Jona + Negro'],
...
function doPost(e) {
  ...
  const headers = ['Timestamp', 'Personaje', 'Actor/a', 'Comentarios'].concat(ENSAYOS.map(item => item[1]));
  ...
  const row = [new Date(), data.personaje || '', data.actor || '', data.comentarios || ''];
